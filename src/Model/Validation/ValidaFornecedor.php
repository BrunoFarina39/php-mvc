<?php 
  namespace Model\Validation;
	...
	class ValidaFornecedor extends AbstractValidation{

		public function __construct(){
      		parent::__construct();
		}

		public function getInputFornecedor(){
      		$validation = function($campos)
      		{
        		...
        		foreach ($campos as $key => $value) {
          			switch ($key) {
            			case 'nome_fantasia':
              				if(empty($value)){
                				$this->inputFilter->setMessage($key,"Campo nome fantasia obrigatório");
                				$retorno = false;
              				}
              				break;        
            			case 'cpf_cnpj':                      
              				...
              				if(strlen($value) == 11 || strlen($value) == 14 || strlen($value) != 0){
                				if($this->validaInsc($value)){
                  					...
                  					if(empty($campos->id)){
                    					if(!$retorno = $this->cpfCnpjFornecedorAdd($value)){
                      						$this->inputFilter->setMessage($key,"Esta inscrição já existe");
                    					}
                  					}else{
                    					if(!$retorno = $this->cpfCnpjFornecedorEdit($value,$campos->id)){
                      						$this->inputFilter->setMessage($key,"Esta inscrição já existe");
                    					}
                  					}                 
               					}else{
    	...
    	private function cpfCnpjFornecedorAdd($value)
   		{
      		$fornecedorDao = new FornecedorDao();
      		return !$fornecedorDao->existeRegistro("fornecedor","cpf_cnpj",$value);   
   		}

    	private function cpfCnpjFornecedorEdit($value,$id)
    	{
      		$fornecedorDao = new FornecedorDao();
      		$reg = $fornecedorDao->buscaRegistro("fornecedor","id",$id);
      
      		if($value != $reg->cpf_cnpj && $fornecedorDao->existeRegistro("fornecedor","cpf_cnpj",$value)){
        		return false;
      		}else{
        		return true;
      		}		     
